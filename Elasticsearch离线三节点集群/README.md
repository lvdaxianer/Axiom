...
helm template es-cluster ./chart \
  ...
  -f ./chart/values.x86.yaml
```

`values.x86.yaml` 使用 `10.100.30.139/library/elasticsearch:7.10.2` 的 AMD64 digest，仅用于 x86 测试。客户 ARM64 环境继续使用 `values.customer.example.yaml` 或自己的 ARM64 镜像覆盖值，不要将 x86 覆盖文件用于 ARM64 Worker。
