...
```json
{
  ...
  "evidenceBindings": [
    ...
    {"target": "then.actions[0]", "evidenceRefs": ["EV-2579-solution-1"]},
    {"target": "then.actions[1]", "evidenceRefs": ["EV-2579-solution-2"]},
...
- 敏感字段在证据展示前按字段级权限脱敏。
- 规则和映射修改必须记录审核人和前后版本。
- API 以认证入口产生的 Principal 为唯一身份来源；客户端选择的 tenant 必须在该 Principal 的授权范围内，并贯穿图谱、版本、运行记录和证据查询。
- 已发布版本通过不可变 manifest 和数据库中的租户级 active pointer 装载；不得通过跨存储的可变别名拼接运行时快照。
